...
class Game:
    ...
    def reset_game(self):
        self.player.resetposisi(100, 375)
        self.jarwo = []
        self.score = 0
        self.jarwo_timer = 100
        self.game_over = False

    def gambarbackground(self):
    ...
    def run(self):
        while self.running:
            ...
            if not self.game_over:
                self.buatmusuh()
                self.player.kecepatan(self.keys, self.width, self.height, self.gambarplayer.get_height(), self.gambarplayer.get_width())
                self.cektabrakan()
                self.score += 1

            self.screen.fill(0)
            self.gambarbackground()
            self.screen.blit(self.gambarplayer, self.player.position)
            for enemy in self.jarwo:
                self.screen.blit(self.gambarjarwo, enemy.position)
            self.textscore()
            if self.game_over:
                self.replay_button_rect = self.gambarreplay()
            pygame.display.flip()
            self.clock.tick(60)
...
class Player:
    ...
    def resetposisi(self, x, y):
        self.position = [x, y]

    def kecepatan(self, keys, screen_width, screen_height, player_height, player_width):
